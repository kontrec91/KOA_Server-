...
export async function userReg(ctx) {
  ...
  const bytes = CryptoJS.AES.decrypt(ctx.request.body.password, process.env.SECRET_KEY as string);
  ...
  if (!userInDB.length) {
    ...
    const token = jwt.sign({ _id: currentUserId.insertedId.toString() }, process.env.ACCESS_KEY as string, { expiresIn: '30s' });//5 minutes
